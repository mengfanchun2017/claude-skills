...
| **自由模式** | 用户说"写 X 报告"无素材 | 无 | 委托 fsearch 搜索 + 调模板 |
...
输出到飞书时，委派 ffeishu skill（工作流 0 创建新文档）。
...
#### Step 大纲.3: 调 fresearchframe 补素材
对每个章节调 fresearchframe 做轻量搜索（2-3 源），产出每章 200-500 字。

#### Step 大纲.4: 填充模板
按模板骨架 + fresearchframe 素材生成报告。
...
#### Step 自由.2: 调 fresearchframe 深度搜索
全流程走 fresearchframe 框架：
- 领域判断
- 三源并行搜索
- 聚合去重
- 输出到 results/
...
> 架构/流程图走 Mermaid 白板（ffeishu 默认）。**数据/分析图走本工作流**——每个图建独立子文档。

### 何时用

| 图类型 | 走法 |
|--------|------|
| 架构图 / 流程图 / 时序图 | Mermaid 白板嵌入（ffeishu） |
